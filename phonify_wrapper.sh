...
awk -v var=${randNum} '{printf "%s\tuniPar_%s/word_%04d.txt\n",$0,var,NR}' $uniParList | parallel --verbose -j $nj --colsep '\t' "valgrind ./unified-parser {1} {2} 1 0 0 0 > /dev/null 2> /dev/null" > /dev/null 2> /dev/null
...
awk -v var=${randNum} '{printf "%s\tuniPar_%s/word_%04d.txt\n",$0,var,NR}' $uniParList | awk '{if(system("[ ! -f " $2 " ]") == 1) {printf("%s\t",$1); system("cat "$2);}}' | sed "s:(set! wordstruct '::g" | sed 's:[)(0 ]::g' > $uniParTemp
rm -rf uniPar_${randNum}
bash get_phone_mapped_text_updated.sh $uniParTemp $outFile
sed -i 's:[" ]::g' $outFile
