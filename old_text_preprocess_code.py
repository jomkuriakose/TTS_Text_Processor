...
class Phonifier:
    ...
    def __phonify(self, text, language, gender):
        ...
        if len(non_dict_words) > 0:
            if(language == 'tamil'):
                ...
                def ppp(word):
                    print(word)
                    new_dict[word] = wordparse(word,0,0,1)
                    print(new_dict[word])
                    print(len(new_dict))
                    print(new_dict)
                # print(Parallel(n_jobs=3)(delayed(wordparse)(i,0,0,1) for i in non_dict_words))
                Parallel(n_jobs=3)(delayed(ppp)(i) for i in non_dict_words)
                print("sadfasdf")
                # for i in non_dict_wo+rds:
                #     # print(i)
    ...
    def __phonifyold(self, text, language, gender):
        ...
        if len(non_dict_words) > 0:
            ...
            if(language == 'tamil'):
                ...
                new_dict_temp = {}
                # print(Parallel(n_jobs=3)(delayed(new_dict_temp[i] = wordparse)(i,0,0,1) for i in non_dict_words))
                print(new_dict_temp)
                print(Parallel(n_jobs=3)(delayed(wordparse)(i,0,0,1) for i in non_dict_words))
            ...
            #         phn_out_dict[eng_words[i]] = self.en_g2p(eng_words[i])
            #     # Create a string representation of the dictionary
            #     data_str = "\n".join([f"{key} {value}" for key, value in phn_out_dict.items()])
            #     print(f"data_str: {data_str}")
            #     if os.path.getsize(out_dict_file) > 0:
            #         with open(out_dict_file, "a") as f:
            #             f.write("\n"+data_str)
            #     else:
            #         with open(out_dict_file, "w") as f:
            #             f.write(data_str)
...
class TextNormalizer:
    def __init__(self, char_map_location=None, phonifier = Phonifier()):
        ...
        for char_map_file in os.listdir(char_map_location):
            try:
                if not char_map_file.endswith(".txt"):
                    continue
                language = char_map_file.split("_")[1].split(".")[0].lower()
                char_map_path = os.path.join(char_map_location, char_map_file)
                df = pd.read_csv(char_map_path, delimiter="\t", header=None, dtype=str)
                self.char_maps[language] = df.set_index(0).to_dict('dict')[1]
            except:
                print("Error")

        print("char maps loaded for the following languages:", list(self.char_maps.keys()))
    ...
    def normalize(self, text, language):
        if language not in self.char_maps:
            print(f"No char map for the given language {language}. No normalization will be done.")
            return self.__post_cleaning(text)

        for char, mapping in self.char_maps[language].items():
            text = re.sub(re.escape(char), mapping, text)
    ...
    def normalize_list(self, text, language):
        ...
        for line in text:
            for char, mapping in self.char_maps[language].items():
                line = re.sub(re.escape(char), mapping, line)
            output_text.append(line)
